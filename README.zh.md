...
## 构建配置
项目当前在 [MemFree.csproj](MemFree/MemFree.csproj) 中定义了一种发布配置：
...
项目支持以下运行时标识符 (RID)：`win-x64`、`win-arm64`、`linux-x64`、`linux-arm64`、`osx-x64`、`osx-arm64`。
可按需使用 JIT 配置进行发布：
...
# macOS ARM64 (Apple Silicon) 自包含单文件发布
...
- 代码遵循现有风格
- 所有平台编译通过 (`dotnet publish -c JIT -r <RID> --self-contained`)
- 更新相关文档
